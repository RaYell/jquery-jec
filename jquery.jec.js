...
msie, optionClasses, optionStyles, position, pref, push, random, remove, removeAttr, removeClass, 
                    ...
                    var parseKeys = function (value) {
                        ...
                        if ($.isArray(value)) {
                            for (i = 0; i < value.length; i += 1) {
                                // min,max tuple
                                if (!$.isArray(value[i]) && Validators.keyRange(value[i])) {
                                    for (j = value[i].min; j <= value[i].max; j += 1) {
                                        keys.push(j);
                                    }
                                // min,max array
                                } else if ($.isArray(value[i]) && Validators.keyRange(value[i])) {
                                    for (j = value[i][0]; j <= value[i][1]; j += 1) {
                                        keys.push(j);
                                    }
                                // number
